use crate::{
    const_helpers::{
        iter, itoa, min_u64, parse_int, read_until, split_with_len, strip_prefix, utf8,
    },
    ConstDaySolution, Day, Input,
};

impl ConstDaySolution for Day<2023, 5> {
...
// const SOLUTION_PART_1: u64 = parse(Day::<2023, 5>::INPUT.as_bytes());
#[allow(long_running_const_eval)]
const SOLUTION_PART_2: u64 = parse2(Day::<2023, 5>::INPUT.as_bytes());
...
const fn parse(input: &[u8]) -> u64 {
    ...
    idx += humidity_to_location_map.len() + DOUBLE_NEWLINE.len();
    ...
    iter! {
        for num in split(seeds, b" ") {
            ...
            res = if res == 0 { location_id } else { min_u64(res, location_id) };
        }
    }
...
const fn map_id<const REVERSE: bool>(id: u64, map: &[u8]) -> u64 {
    iter! {
        for line in lines(map) {
            ...
            let dst_range_start = if REVERSE { parse_int(src_range_start_bz) as u64 } else { parse_int(dst_range_start_bz) as u64 };
            let src_range_start = if REVERSE { parse_int(dst_range_start_bz) as u64 } else { parse_int(src_range_start_bz) as u64 };
